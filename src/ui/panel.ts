...
import { stopsForDay } from '../data/trip';
...
export function renderPanel(
  ...
  const parts: string[] = [];
  parts.push(selectionHeader(track, day, stops));
  parts.push(statTiles(agg));
  ...
  parts.push(municipalityChips(track));

  parts.push(
    `<h2>Stopp og varighet${day === null ? '' : ' denne dagen'}</h2>`,
  );
  parts.push(
    '<p class="footnote">Mørk fiolett = overnattingsstopp (850–1250 min).</p>',
  );
...
function selectionHeader(
  ...
  return (
    ...
    `<p class="selection-sub" id="selection-sub">${formatDayLong(day)} · ${stops.length} stopp</p>`
  );
}

function statTiles(agg: Aggregate): string {
  return (
    '<div class="stat-tiles">' +
    '<div class="stat-tile"><div class="stat-label">Distanse</div>' +
    `<div class="stat-value" id="stat-distance">${formatKm(agg.total.distanceKm)}</div></div>` +
    '<div class="stat-tile"><div class="stat-label">Tid i bevegelse</div>' +
    `<div class="stat-value" id="stat-duration">${formatDuration(agg.total.durationMin)}</div></div>` +
    '</div>'
  );
}
...
function modeTable(agg: Aggregate): string {
  const rows = [...agg.byMode.entries()]
    .map(([mode, totals]) => {
      const dot = MODE_DOT_COLORS[mode] ?? '#898781';
      return (
        `<tr class="mode-row" data-mode="${escapeXml(mode)}">` +
        `<td><span class="mode-dot" style="background:${dot}"></span>${escapeXml(modeLabel(mode))}</td>` +
        `<td>${totals.legs}</td>` +
...
function municipalityChips(track: DayTrack | null | undefined): string {
  if (track) {
    const chips = track.municipalities
      .map((m) => `<span class="muni-chip">${escapeXml(m)}</span>`)
      .join('');
    return (
      `<h2>Kommuner denne dagen (${track.municipalities.length})</h2>` +
      '<p class="footnote">I rekkefølge langs ruta.</p>' +
  ...
  const rows = legSpeeds(legs)
    .map((leg) => {
      return (
        ...
        `<td>${escapeXml(modeLabel(leg.mode))}</td>` +
